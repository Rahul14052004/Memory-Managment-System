...
/*void join_nodes(struct sub_node *ptr){
    struct sub_node *start = ptr;
    while(start!=NULL){
        if(start->type[0] == 'H'){
            if(start->next->type[0]=='H' && start->prev->type[0]){
                start
            }
        }
    }
...
void mems_print_stats(){
    ...
    while(ptr!=NULL){
        ...
        printf("MAIN[%d:%d]->",ptr->s_va,ptr->e_va);
        struct sub_node *ptr2 = ptr->sub_head;
        int c=0;
        while(ptr2!=NULL){
            if(ptr2->type[0] =='H'){
            ...
            c++;
            printf("%c[%d:%d]<->",ptr2->type[0],ptr2->s_va,ptr2->e_va);
            ptr2 = ptr2->next;
        }
        printf("NULL\n");
        sub_size[iter]=c;
